...
def get_stats(wildcards):
    import glob, os
    stats = glob.glob(os.path.join("{wildcards.exp}/clusterings_pca/*/", "*_performance.csv"))
    return stats

def get_ext_val_done(wildcards):
    import glob, os
    pl = glob.glob(os.path.join(DATASET_DIR, "snake_workflow", "{wildcards.exp}-{wildcards.tool}-{wildcards.sample}--external-validation-pca.done"))
    return pl

_data_folder_ = "{exp}/data"
_out_folder_ = "{exp}/clusterings_pca"
_plh_dir_ = "snake_workflow"


rule run_tools:
    output:
        touch(_plh_dir_+'/clustering-pca.done')
    input:
...
rule run_affinity:
    ...
    input:
        sim_file = _data_folder_ + '/{sample}/gt_merged/principal_components.csv'
    params:
        prgm = TOOLS['affinity'],
...
rule run_dbscan:
    output:
         _out_folder_ + '/dbscan/{sample}_dbscan_clusters.csv'
    input:
        sim_file = _data_folder_ + '/{sample}/gt_merged/principal_components.csv'
    params:
        prgm = TOOLS['dbscan'],
...
rule run_hdbscan:
    output:
        _out_folder_ + '/hdbscan/{sample}_hdbscan_clusters.csv'
    input:
        sim_file = _data_folder_ + '/{sample}/gt_merged/principal_components.csv'
    params:
        prgm = TOOLS['hdbscan'],
...
rule stability_score:
    input: prev_step = _plh_dir_+'/clustering-pca.done',
        stats = get_stats,
        file = _data_folder_ + "/{sample}/gt_merged/SegCopy",      
    output:
        touch(_plh_dir_+"/{exp}-{tool}-{sample}-stability-pca.done")
    params: 
...
rule ext_validation:
    input:
        prev_step= _plh_dir_+"/{exp}-{tool}-{sample}-stability-pca.done",
        true_labels = _data_folder_+"/{sample}/gt_merged/tree_clusters.csv"
    output:
        touch(_plh_dir_+"/{exp}-{tool}-{sample}-external-validation-pca.done")
    params:
        prgm =  EXT_VALIDATION,
...
rule all:
    ...
    output: touch(_plh_dir_+"/clustering_evaluation-pca.done")